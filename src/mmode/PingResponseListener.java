			...
			new PacketAdapter(
				...
				public void onPacketSending(PacketEvent event) {
					try {
						...
						// set ping message
						String pingMessage = ChatColor.translateAlternateColorCodes('&', config.mmodeMessage);
						ping.setVersionProtocol(-1);
						ping.setVersionName(pingMessage);
						// set motd
						String prevmotd = ping.getMotD().getJson();
						String motd = ChatColor.translateAlternateColorCodes('&', config.mmodeMOTD.replace("{motd}", prevmotd));
						ping.setMotD(motd);
						// set icon
